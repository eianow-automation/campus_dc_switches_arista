...
This folder contains `srl-quickstart.clab.yml`, which uses a freely downloadable image to test your Docker and Containerlab installation.
...
```bash
mkdir -p clab_topo/srl-quickstart && cd clab_topo/srl-quickstart && curl -O https://raw.githubusercontent.com/eianow-automation/campus_dc_switches_arista/main/quickstart/srl-quickstart.clab.yml
```
...
3. Download the `srl-quickstart.clab.yml` file (ensure it's placed in the `srl-quickstart` directory):
   - Via browser: [GitHub](https://github.com/eianow-automation/campus_dc_switches_arista/blob/main/quickstart/srl-quickstart.clab.yml) (download raw file and save it in the `srl-quickstart` directory)
   - Via CLI:
     ```bash
     curl -O https://raw.githubusercontent.com/eianow-automation/campus_dc_switches_arista/main/quickstart/srl-quickstart.clab.yml
     ```

4. Start the topology:
   ```bash
   clab deploy -t srl-quickstart.clab.yml
   ```
...
5. Log into srl01:
   ...
   - Using SSH (username: `admin`, password: `admin`):
     ...
     ```bash
     clab inspect -t srl-quickstart.clab.yml
     ```
     Then SSH:
